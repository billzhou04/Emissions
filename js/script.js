const DATA_URL = './data/OECD.ENV.EPI,DSD_AIR_GHG@DF_AIR_GHG,+.A.GHG._T.KG_CO2E_PS.csv';
const START_YEAR = 2014;
const END_YEAR = 2021;
const TARGET_MEASURE = '_T';
const TARGET_UNIT = 'KG_CO2E_PS';

const selectedCountriesWhite = ['USA', 'CAN', 'AUS', 'DEU', 'FRA', 'GBR', 'JPN', 'KOR', 'CHE', 'CHL', 'MEX', 'CRI', 'KAZ'];
...
function loadData() {
    d3.csv(DATA_URL).then(data => {
        ...
        const dataGroupedWhite = d3.group(filteredData.filter(d => selectedCountriesWhite.includes(d.countryCode)), d => d.countryName);
        dataGroupedWhite.forEach(group => group.sort((a, b) => a.year - b.year));


        const dataBlackRaw = filteredData.filter(d => selectedCountriesBlack.includes(d.countryCode));
         const dataPivotedBlack = Array.from(d3.group(dataBlackRaw, d => d.yearNum)).map(([yearNum, values]) => {
        ...
        if (dataGroupedWhite.size > 0) {
            ...
            d3.select("#white-hat-vis").html("<p>Error: No data available for the White Hat selection.</p>");
        }
        ...
        if (dataPivotedBlack.length > 0) {
             ...
             d3.select("#black-hat-vis").html("<p>Error: No data available for the Black Hat selection.</p>");
        }
        ...
        d3.select(".container").insert("p", ":first-child")
           ...
           .text(`Failed to load data from ${DATA_URL}. Please check file path and server.`);
    });
}
...
function createWhiteHatChart(dataGrouped) {
    ...
    const timeDomain = d3.extent(Array.from(dataGrouped.values())[0], d => d.year);
    ...
    function mousemoveHandler(event) {
        ...
        let tooltipContent = `<div style="font-weight: bold;">${d3.timeFormat("%Y")(x0)}</div>`;
        let closestPoints = [];
        ...
        dataGrouped.forEach((values, key) => {
            ...
            if(d) closestPoints.push({ country: key, value: d.actualValue, year: d.year, color: color(key) });
        });
        ...
        path.classed("highlight", d => d[0] === closestCountry)
            .classed("fade", d => d[0] !== closestCountry);

        closestPoints.forEach(p => {
            tooltipContent += `
                <div style="color: ${p.color}; ${p.country === closestCountry ? 'font-weight: bold;' : ''}">
                  ${p.country}: ${d3.format(",.1f")(p.value)}
                </div>`;
        });

        const closestYearOverall = closestPoints.length > 0 ? closestPoints[0].year : x0;
        focus.select(".hover-line").attr("transform", `translate(${x(closestYearOverall)},0)`);
...
function createBlackHatChart(dataPivoted) {
    ...
    const svg = d3.select("#black-hat-vis")
        .append("svg")
